...
x3dbuf = StringIO(dgm.GetMetaDatumWIStr(landmarked_3d_wfmdict[landmarked_3d_x3dchan],"X3DGeom",""))
...
th=Thread(target=lambda: imageprojection_float(projparams,imagedata))
th.start()
th.join()


refres = paramwfm.data.copy()
refval = validitybuf.copy()
...
for cnt in range(15):
    ...
    if not(((refres == paramwfm.data) | (np.isnan(refres) & np.isnan(paramwfm.data))).all()) or (refval != validitybuf).any():
        different = ~((refres == paramwfm.data) | (np.isnan(refres) & np.isnan(paramwfm.data)))
        differentvalidity = (refval != validitybuf)

        refresdiff=refres[different]
        paramwfmdiff=paramwfm.data[different]
        refvaldiff=refval[differentvalidity]
        validitybufdiff=validitybuf[differentvalidity]
        
        print(np.where(different))
        print(np.where(differentvalidity))
        if paramwfmdiff.shape[0] > 0:
            print(np.linalg.norm(refresdiff-paramwfmdiff)/np.linalg.norm(paramwfmdiff))
            print(np.max(np.abs(refresdiff-paramwfmdiff))/np.linalg.norm(paramwfmdiff))
            pass
        else:
            print(0.0)
            print(0.0)
        ...
        if refvaldiff.shape[0] > 0:
            print(np.linalg.norm(refvaldiff-validitybufdiff)/np.linalg.norm(refvaldiff))
            print(np.max(np.abs(refvaldiff-validitybufdiff))/np.linalg.norm(refvaldiff))
            pass
        else:
            print(0.0)
            print(0.0)
